                    ...
                    <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr 1fr":"repeat(4,1fr)",gap:12}}>
                      ...
                      <div style={{borderRadius:14,background:"linear-gradient(135deg,#2e1065,#5b21b6,#7c3aed)",padding:"16px",position:"relative",overflow:"hidden",boxShadow:"0 6px 18px rgba(124,58,237,0.25)"}}>
                        ...
                        <div style={{position:"relative",zIndex:1}}>
                          ...
                          <p style={{margin:"0 0 4px",fontSize:monthRefunds>0?22:28,fontWeight:900,color:"white",fontFamily:"DM Mono,monospace",letterSpacing:"-0.5px"}}>{monthRefunds>0?fmt(shopId,monthRefunds):"—"}</p>
    ...
    <div style={{display:"flex",flexDirection:"column",maxHeight:"72vh",overflowY:"auto"}}>
      ...
      <div style={{marginBottom:20}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
          ...
          <div>
            ...
            <input value={form.invoiceNo} readOnly style={{...inp,background:"#f8fafc",fontFamily:"DM Mono,monospace",fontWeight:700,fontSize:12,color:shop.accent,cursor:"default",border:"1px solid #e2e8f0"}}/>
          </div>
        </div>
      </div>
        ...
        {hasLines&&(
          <div style={{background:"#f8fafc",border:"1px solid #e2e8f0",borderRadius:12,padding:"14px 16px",marginBottom:14}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
              ...
              <button type="button" onClick={addLine} style={{fontSize:11,fontWeight:700,color:shop.accent,background:shop.accentBg,border:"1px solid "+shop.accent+"44",borderRadius:6,padding:"4px 12px",cursor:"pointer",fontFamily:"inherit"}}>+ Add Item</button>
            </div>
            ...
            {editLines.map((l,i)=>{
              ...
              return(
                <div key={i} style={{display:"grid",gridTemplateColumns:"1fr 60px 90px 70px 28px",gap:4,marginBottom:6,alignItems:"center"}}>
                  <input value={l.name} onChange={e=>setLine(i,"name",e.target.value)} placeholder="Item name" style={{...inp,fontSize:12,padding:"6px 9px"}} onFocus={fo} onBlur={bl}/>
                  <input type="number" onWheel={e=>e.target.blur()} min="1" value={l.qty} onChange={e=>setLine(i,"qty",e.target.value)} style={{...inp,fontSize:12,padding:"6px 6px",textAlign:"right"}} onFocus={fo} onBlur={bl}/>
                  <input type="number" onWheel={e=>e.target.blur()} min="0" step="0.01" value={l.price} onChange={e=>setLine(i,"price",e.target.value)} placeholder="0.00" style={{...inp,fontSize:12,padding:"6px 6px",textAlign:"right"}} onFocus={fo} onBlur={bl}/>
                  <span style={{fontSize:12,fontWeight:700,color:shop.accent,textAlign:"right",paddingRight:4}}>{shop.symbol}{lineTotal.toLocaleString()}</span>
                  <button type="button" onClick={()=>removeLine(i)} title="Remove item" style={{width:24,height:24,borderRadius:6,border:"1px solid #fecaca",background:"#fff5f5",color:"#dc2626",cursor:"pointer",fontSize:14,display:"flex",alignItems:"center",justifyContent:"center",lineHeight:1,padding:0,fontFamily:"inherit"}}>×</button>
                </div>
              );
            })}
            {editLines.length===0&&(<p style={{margin:"4px 0 0",fontSize:11,color:"#94a3b8",fontStyle:"italic"}}>No items — click "+ Add Item" to add one.</p>)}
          </div>
        )}
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
          ...
          <div style={{gridColumn:"1/-1"}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
                ...
                {[0,5,10,18,20].map(r=>(
                  <button key={r} type="button" onClick={()=>set("taxRate",Number(r))}
                    style={{padding:"4px 12px",borderRadius:999,border:"2px solid "+(Number(form.taxRate)===r?shop.accent:"#e2e8f0"),background:Number(form.taxRate)===r?shop.accent:"white",color:Number(form.taxRate)===r?"white":"#374151",fontWeight:700,fontSize:12,cursor:"pointer",fontFamily:"inherit",transition:"all 0.15s"}}>
                    {r}%
                  </button>
                ))}
              </div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
              <div>
                <label style={lbl}>Grand Total ({shop.symbol}) — {form.taxRate===0?"no tax":form.taxInclusive?"incl. tax":"excl. tax"}</label>
                <input type="number" onWheel={e=>e.target.blur()} value={form.amount} onChange={e=>set("amount",e.target.value)} placeholder="0.00" style={inp} onFocus={fo} onBlur={bl}/>
              </div>
              {form.amount&&Number(form.amount)>0&&(()=>{
                ...
                return(
                  <div style={{background:"#f8fafc",borderRadius:10,padding:"12px 14px",border:"1px solid #e2e8f0"}}>
                    ...
                    <div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}><span style={{fontSize:11,color:"#64748b"}}>Subtotal</span><span style={{fontSize:11,fontWeight:700,color:"#374151"}}>{shop.symbol}{subtotal.toLocaleString()}</span></div>
                    <div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}><span style={{fontSize:11,color:"#64748b"}}>{(form.taxRate||0)===0?"No Tax":"Tax "+form.taxRate+"%"}</span><span style={{fontSize:11,fontWeight:700,color:"#374151"}}>{shop.symbol}{tax.toLocaleString()}</span></div>
                    <div style={{display:"flex",justifyContent:"space-between",borderTop:"1px solid #e2e8f0",paddingTop:4,marginTop:4}}><span style={{fontSize:12,fontWeight:800,color:"#0f172a"}}>Grand Total</span><span style={{fontSize:12,fontWeight:900,color:shop.accent}}>{shop.symbol}{grand.toLocaleString()}</span></div>
                  </div>
                );
              })()}
            </div>
      ...
      <div style={{marginBottom:20}}>
        ...
        <div style={{background:"#f8fafc",borderRadius:14,padding:"16px",border:"1px solid #e2e8f0"}}>
          <div style={{display:"grid",gridTemplateColumns:form.payBy==="SHOP"?"1fr 1fr":"1fr",gap:14}}>
            <div>
            ...
            {form.payBy==="SHOP"&&(
              <div>
                ...
                <input value={form.shopInvoiceNo||""} onChange={e=>set("shopInvoiceNo",e.target.value)} placeholder="Shop invoice number…" style={{...inp,fontFamily:"DM Mono,monospace"}} onFocus={fo} onBlur={bl}/>
              </div>
            )}
          </div>
        </div>
          ...
          <div style={{background:"#f0fdf4",borderRadius:14,padding:"16px",border:"1px solid #bbf7d0"}}>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:14}}>
              <div>
                ...
                <input value={form.purInvNo} onChange={e=>set("purInvNo",e.target.value)} placeholder="Supplier invoice no." style={{...inp,border:"1px solid #86efac"}} onFocus={fo} onBlur={bl}/>
              </div>
              <div>
                <label style={{...lbl,color:"#166534"}}>📅 Pur. Inv. Date</label>
                <input type="date" value={form.purInvDate} onChange={e=>set("purInvDate",e.target.value)} style={{...inp,border:"1px solid #86efac"}} onFocus={fo} onBlur={bl}/>
              </div>
            </div>
            <div>
              <label style={{...lbl,color:"#166534"}}>💰 Pur. Amount ({shop.symbol})</label>
              <input type="number" onWheel={e=>e.target.blur()} value={form.purAmount} onChange={e=>set("purAmount",e.target.value)} placeholder="0.00" style={{...inp,border:"1px solid #86efac"}} onFocus={fo} onBlur={bl}/>
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{flexShrink:0,display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,padding:"14px 24px",borderTop:"1px solid #f1f5f9",background:"white",boxShadow:"0 -2px 12px rgba(0,0,0,0.05)"}}>
        ...
        <button onClick={onClose} style={{padding:"13px 0",borderRadius:12,border:"1px solid #e2e8f0",background:"white",color:"#374151",fontWeight:700,fontSize:14,cursor:"pointer",fontFamily:"inherit"}}>
          Cancel
        </button>
      </div>
    </div>
    ...
    <div style={{display:"flex",flexDirection:"column",height:"100%"}}>
      <div style={{flex:1,overflowY:"auto",padding:"12px 16px",WebkitOverflowScrolling:"touch"}}>
            ...
            <div style={{background:"#f8fafc",borderRadius:12,padding:"11px 12px",marginBottom:8,border:"1px solid #f1f5f9"}}>
              ...
              {form.payBy==="SHOP"&&(
                <div style={{marginBottom:7}}><label style={lbl}>Shop Invoice No.</label><input value={form.shopInvoiceNo} onChange={e=>set("shopInvoiceNo",e.target.value)} placeholder="Shop invoice number…" style={{...inp,fontFamily:"DM Mono,monospace"}} onFocus={fo} onBlur={bl}/></div>
              )}
