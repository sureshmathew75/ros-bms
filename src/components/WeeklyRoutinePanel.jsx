...
import { dbLoadInventoryItems, dbAddInventoryMovement, dbLoadWeeklyRoutine, dbSaveWeeklyRoutine } from "../db";
...
/* ─────────────────────────────────────────────────────────────────────────
   ...
   here touches the `products` table used by the UK shops.
   ───────────────────────────────────────────────────────────────────────── */
...
export default function WeeklyRoutinePanel({ shopId, shop, user, rosieTasks = [], isRosieTaskDue, onMarkTaskDone, staffAccounts = [], onAddTask }) {
  const myId = user?.id || "";
  ...
  const countedCount = routine.stockItems.filter(si => si.countedQty !== null).length;
  const checkedCount = routine.docChecks.filter(c => c.checked).length;
  ...
  return (
    <div style={{ maxWidth: 820, margin: "0 auto" }}>
      <div style={{ marginBottom: 18 }}>
        ...
        <p style={{ margin: 0, fontSize: 12.5, color: "#94a3b8" }}>{fmtSaturday(weekEnding)}</p>
      </div>
